// src/contexts/AIContext.tsx - COMPLETE AI INTEGRATION (FIXED)
import React, {
  createContext,
  useContext,
  useReducer,
...
function aiReducer(state: AIState, action: AIAction): AIState {
  switch (action.type) {
    ...
    case "ADD_INSIGHT":
      return {
        ...state,
        insights: [action.payload, ...state.insights.slice(0, 49)],
      }; // Keep last 50
...
export function AIContextProvider({ children }: { children: React.ReactNode }) {
  ...
  const analyzeProduction = useCallback(async (data: any) => {
  ...
  const generateInsights = useCallback(async (data: any) => {
    try {
      ...
      if (result.suggestions) {
        const insights: AIInsight[] = result.suggestions.map(
          (suggestion: any, index: number) => ({
            id: `insight-${Date.now()}-${index}`,
            type: "efficiency",
            title: suggestion.label,
            description: `AI-generated insight with ${(suggestion.confidence * 100).toFixed(
              1
            )}% confidence`,
  ...
  const askAI = useCallback(async (question: string): Promise<string> => {
    try {
      ...
      const result = await response.json();
      return (
        result.text || "Sorry, I could not generate a response at this time."
      );
    } catch (error) {
  ...
  const summarizeData = useCallback(
    async (data: any, length: "short" | "medium" | "long" = "medium") => {
      try {
...
async function generateProductionInsights(data: any): Promise<AIInsight[]> {
  ...
  if (data.operators && data.operators.length > 0) {
    const avgEfficiency =
      data.operators.reduce((sum: number, op: any) => sum + op.efficiency, 0) /
      data.operators.length;
    const lowPerformers = data.operators.filter(
      (op: any) => op.efficiency < 70
    ).length;
    const highPerformers = data.operators.filter(
      (op: any) => op.efficiency >= 100
    ).length;
    ...
    insights.push({
      ...
      description: `Average efficiency is ${avgEfficiency.toFixed(
        1
      )}%. ${highPerformers} high performers, ${lowPerformers} need attention.`,
      confidence: 0.95,
...
async function generatePredictions(data: any): Promise<AIState["predictions"]> {
  ...
  const avgEfficiency =
    ...
    data.operators.length;
  const trend =
    avgEfficiency > 85 ? "increasing" : avgEfficiency < 70 ? "decreasing" : "stable";

  return {
    efficiency: Math.min(
      100,
      avgEfficiency + (trend === "increasing" ? 5 : trend === "decreasing" ? -5 : 0)
    ),
    trend,
    confidence: 0.85,
    timeframe: "1 hour",
  ...
  if (data.operators && data.operators.length > 0) {
    ...
    if (lowPerformers.length > 0) {
      recommendations.push(
        `Provide additional training for ${lowPerformers.length} underperforming operators`
      );
      recommendations.push(
        "Review work allocation and machine maintenance schedules"
      );
    }

    if (avgEfficiency < 80) {
      recommendations.push(
        "Consider workflow optimization to improve overall efficiency"
      );
    ...
    if (data.underperformers && data.underperformers.length > 0) {
      recommendations.push(
        "Schedule immediate supervisor intervention for critical cases"
      );
    }
  }
